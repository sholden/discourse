...
var MD = window.BetterMarkdown.Markdown,
    urlPattern = /^((?:[a-z][\w\-]+:(?:\/{1,3}|[a-z0-9%])|www\d{0,3}[.])(?:[^\s()<>]+|\([^\s()<>]+\))+(?:\([^\s()<>]+\)|[^`!()\[\]{};:'".,<>?«»“”‘’\s]))/;

var dialect = MD.dialects.Discourse = MD.subclassDialect( MD.dialects.Gruber );

// Autolinks
dialect.inline["http"] = function(text, orig_match) {
  var matches = urlPattern.exec(text);
  if (matches && matches[0]) {
    return [matches[0].length, ['a', {href: matches[0]}, matches[0]]];
  }
};
...
dialect.inline["[quote="] = function(text, orig_match) {
  ...
  // var bbcodePattern = /\[quote=([^\]]*)\]((?:[\s\S](?!\[quote=[^\]]*\]))*?)\[\/quote\]/im
